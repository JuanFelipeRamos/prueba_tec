...
const borrarNota = async () => {
  ...
  try {
    await api.delete(`/notas/notas/${props.nota.id}/`, {
      headers: {
    ...
    alert('Error al borrar nota. Inténtalo nuevamente.')
  }
}
</script>
  ...
  <div v-if="mostrar" class="modal-overlay" @click.self="cerrar">
    <section class="modal-card" role="dialog" aria-modal="true" aria-labelledby="titulo-confirmar-estado">
      <header class="modal-header">
        <h2 id="titulo-confirmar-estado" class="modal-titulo">¿Deseas borrar la nota <strong>{{ nota?.titulo }}</strong>?</h2>
      </header>
